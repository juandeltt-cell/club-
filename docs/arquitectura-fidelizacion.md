...
**Coexistencia** permite conectar a la API el **mismo número** que el restaurante ya usa en la app WhatsApp Business: sigue chateando desde la app y el sistema manda por API. Es la experiencia ideal, pero se habilita mediante *Embedded Signup*, y para eso Luz Sur tiene que registrarse como **Tech Provider** de Meta (gratis, con verificación de negocio y revisión de la app, de semanas) — **no es algo que un negocio pueda activar por su cuenta sin un partner certificado**. Incluso una vez habilitada, tiene una condición operativa a tener en cuenta: **si nadie abre la app de WhatsApp Business en ese celular durante 7 días seguidos, la conexión con la API se corta** y hay que volver a vincularla. En un restaurante donde la app la usan a diario para pedidos y reservas no es un problema; si el número solo se usa esporádicamente, sí puede serlo.
