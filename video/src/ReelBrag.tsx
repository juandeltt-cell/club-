...
const cuts = [S.claim, S.who, S.qr, S.stars, S.prize, S.commerce, S.know, S.slowQ, S.slowA, S.aiQ, S.aiMsg, S.ret, S.punch, S.closing];
const SFX: Fx[] = [
  ...
  [S.qr.from + 30, "sel", 0.2], [S.stars.from + 6, "shimmer", 0.26], ...[0, 1, 2, 3, 4].map((i): Fx => [S.stars.from + 22 + i * 7, "chip", 0.2]),
  [S.prize.from + 8, "card", 0.24], [S.prize.from + 12, "ding", 0.26],
  [S.slowA.from + 26, "cut2", 0.18], [S.slowA.from + 52, "pop", 0.26],
  ...Array.from({ length: 18 }, (_, i): Fx => [S.aiMsg.from + 8 + i * 3, "key", 0.1]),
  [S.aiMsg.from + 66, "pop", 0.22], [S.aiMsg.from + 90, "tap", 0.45], [S.aiMsg.from + 96, "ding", 0.3],
  [S.punch.from, "hit", 0.32], [S.punch.from + 13, "hit", 0.28], [S.punch.from + 26, "hit", 0.3],
  ...
  return (
    <AbsoluteFill style={{ background: T.ink }}>
      <AudioReactiveProvider src="music/song-reel.wav">
        ...
        <Scene s={S.who} dir="left"><Sc.HookWho /></Scene>
        <Scene s={S.logo} dir="none"><LogoDrop /></Scene>
        <Scene s={S.qr}><Sc.StepQR /></Scene>
